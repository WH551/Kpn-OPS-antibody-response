...
def bootstrap_ci_median(data, alpha=0.05, n_bootstraps=10000):
...
def mannwhittest(df, column, group_column, alpha=0.05):
    ...
    for i, first_group in enumerate(groups):
        ...
        ci_lower, ci_upper = bootstrap_ci_median(data1)
    ...
    return pd.DataFrame(p_value_results), pd.DataFrame(CI_results)
...
for outerind, ax in enumerate(axs.flat):
    ...
    result_df, CI_df = mannwhittest(combined_hc_bc_kpn, column=final_antigen_list[outerind], group_column='Group_Condition')

    # Display the results
    print(final_antigen_list[outerind])
    print(result_df)
    print(CI_df)
    print("========================================================================")
...
for outerind, ax in enumerate(axs.flat):
    ...
    # Perform Mann-Whitney U test and calculate 95% confidence intervals
    result_df, CI_df = mannwhittest(combined_hc_bc_kpn, column=final_antigen_list[outerind], group_column='Classification')
