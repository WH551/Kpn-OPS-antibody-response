...
for outerind, subfig in enumerate(subfigs.flat):
    ...
    if outerind == 0 :
        q.set_ylabel("MFI", fontsize = 14, weight = 'bold')
    if outerind >= 0 :
        p.set_xlabel(None)
...
fig.tight_layout()
plt.savefig(result_folder+"Spagetti_plot_O1v1,O1v2,O3b.png", dpi=500, transparent=True, bbox_inches='tight')
...
fig.tight_layout()
plt.savefig(result_folder+"Spagetti_plot_O2v1,O2v2,O3,O5.png", dpi=500, transparent=True, bbox_inches='tight')
...
for antigen in final_antigen_list:
    ...
    for outerind, ax in enumerate(axs.flat):
        ...
        ax.set_xlabel('')
        if outerind == 0:
            ax.set_ylabel("MFI", fontsize=14, weight='bold')
        else :
    ...
    plt.savefig(result_folder+f"Brief_Spagetti_plot_{antigen_name}_KPN_isolates.png", dpi=500, transparent=True, bbox_inches='tight')
...
for outerind, subfig in enumerate(subfigs.flat):
    ...
    if outerind == 0:
        axs[0].set_ylabel("MFI", fontsize=14, weight='bold')
        axs[1].tick_params(left=False)

fig.tight_layout()
plt.savefig(result_folder+"Spagetti_plot_O1Kpn_I_nonI.png", dpi=300, transparent=True, bbox_inches='tight')
...
for outerind, ax in enumerate(axs.flat):
    ...
    if outerind > 0:
        ...
        ax.set_ylabel("Fold change", fontsize=14, weight='bold')
    
fig.tight_layout()
plt.savefig(result_folder+"Boxplot_O1Kpn_I_nonI.png", dpi=300, transparent=True, bbox_inches='tight')
...
fig.tight_layout()
plt.savefig(result_folder+"Boxplots_cross_reactive_antibody_response_to_O1,O3_antigens.png", dpi=300, transparent=True, bbox_inches='tight')
...
fig.tight_layout()
plt.savefig(result_folder+"Boxplots_cross_reactive_antibody_response_to_O2,O5,MrkA_antigens.png", dpi=300, transparent=True, bbox_inches='tight')
...
for outerind, ax in enumerate(axs.flat):
    ...
    if outerind > 0:
        ...
        ax.set_ylabel("Fold change", fontsize=12, weight='bold')

fig.tight_layout()
plt.savefig(result_folder+"Boxplots_antibody_responses_to_homologousOPS_and_MrkA.png", dpi=300, transparent=True, bbox_inches='tight')
...
g = sns.heatmap(correlation_df, annot=annotation_data, annot_kws={"size": 15}, vmax=1, vmin=-1, cmap=colormap, mask=mask, fmt='s')
...
plt.tight_layout()
plt.savefig(result_folder+"Cross-specificity_regression_analysis_heatmap.png", dpi=300, transparent=True, bbox_inches='tight')

#============================================================================================#
#============================================================================================#
# Figure S10

# List of samples with MrkA positive values
MrkA_positive_sample_list = [
    index for index in combined_data_highest_value.index 
    if combined_data_highest_value["MrkA"][index] > 0
]

# Dataframe of MrkA positive samples with fold change
MrkA_positive_df_foldchange = combined_data_foldchange.loc[MrkA_positive_sample_list]

# Dataframe to store the highest fold change for each group
MrkA_highest_df_foldchange = pd.DataFrame(columns=MrkA_positive_df_foldchange.columns)

# Find the sample with the highest MrkA fold change for each group
for key, df in MrkA_positive_df_foldchange.groupby("Group"):
    if "KPN" in key:
        max_fold = df["MrkA"].idxmax()
        MrkA_highest_df_foldchange.loc[key] = MrkA_positive_df_foldchange.loc[max_fold]

# Columns to check
columns_to_check = ["O1v1-EPA", "O1v2-HSA", "O2v1-HSA", "O2v2-EPA", "O3/O3a-HSA", "O3b-EPA", "O5-HSA"]

# Iterate through columns and filter based on upper fence criteria
for column in columns_to_check:
    ops_hc_data = combined_data_foldchange.loc[combined_data_foldchange.index.str.contains("HC"), column]
    mrkA_hc_data = combined_data_foldchange.loc[combined_data_foldchange.index.str.contains("HC"), "MrkA"]
    ops_upper_fence = calculate_upper_fence(ops_hc_data)
    mrkA_upper_fence = calculate_upper_fence(mrkA_hc_data)
    
    for index in MrkA_highest_df_foldchange.index:
        antigen = column.split("-")[0]
        if MrkA_highest_df_foldchange["O-type"][index] == antigen:
            if (MrkA_highest_df_foldchange[column][index] < ops_upper_fence and 
                MrkA_highest_df_foldchange["MrkA"][index] < mrkA_upper_fence):
                MrkA_highest_df_foldchange.drop(index, inplace=True)
...
fig, axs = plt.subplots(1, 2, figsize=(17, 7), sharey=False, gridspec_kw={'wspace': 0.2})

def plot_scatter(ax, condition_df, legend_title):
    extra_text = {}
    for o_type, o_type_df in condition_df.groupby("O-type"):
        if o_type in o_antigen_to_epa_hsa:
            filtered_df = o_type_df[o_type_df[o_antigen_to_epa_hsa[o_type]] > 0]
            if not filtered_df.empty:
                extra_text[o_type] = f" (N={len(filtered_df)})"
                sns.scatterplot(
                    x="MrkA",
                    y=o_antigen_to_epa_hsa[o_type],
                    data=filtered_df,
                    color=o_antigen_color_dict[o_type],
                    edgecolor="black",
                    linewidth=0.5,
                    s=60,
                    ax=ax
    ...
    ax.set_xlabel("Fold change to MrkA", fontsize=18)
    ax.set_ylabel("Fold change to homologous OPS", fontsize=18)
    ax.tick_params(axis='y', labelsize=16)
    ax.tick_params(axis='x', labelsize=16)
    ax.plot([0, 50], [0, 50], ls="--", c="gray")

    handles = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor=color, markeredgecolor='black', markeredgewidth=0.5, markersize=10, label=label + extra_text.get(label, ""))
        for label, color in o_antigen_color_dict.items()
    ]
    ax.legend(handles=handles, title=legend_title, loc='upper right')

# Plot for each condition
for condition, condition_df in MrkA_highest_df_foldchange.groupby("Condition"):
    ax_index = 0 if condition == "I" else 1
    plot_scatter(axs[ax_index], condition_df, "Kpn infection")

plt.tight_layout()
plt.savefig(result_folder+"MrkA_OPS_fold_change_I_nonI.png", transparent=True, dpi=500, bbox_inches='tight')
...
# Function to plot Kendall correlation and bootstrap confidence intervals
def plot_kendall_correlation(df, x_col, y_col, title, filename):
    sns.set_style('ticks')
    fig, ax = plt.subplots(1, 1, figsize=(7, 7))

    correlation, p_value = kendalltau(df[x_col], df[y_col])
    ...
    for _ in range(n_bootstraps):
        ...
        bootstrap_corr, _ = kendalltau(x_bootstrap, y_bootstrap)
        bootstrap_correlations.append(bootstrap_corr)
    ...
    ax.set_xlabel("Glucuronic acid", size=14, weight='bold')
    ax.set_ylabel("MFI", size=14, weight='bold')
...
# Plot for O1v1-EPA
plot_kendall_correlation(O1Kpn_O1v1EPA_df, "Glucuronic_acid", "O1v1-EPA", "O1v1-EPA Correlation", "O1Kpn_O1v1_EPA_capsule_Kendall.png")

# Plot for O1v2-HSA
plot_kendall_correlation(O1Kpn_O1v2HSA_df, "Glucuronic_acid", "O1v2-HSA", "O1v2-HSA Correlation", "O1Kpn_O1v2_HSA_capsule_Kendall.png")
...
p.set_ylabel("Fold change",size = 14)
p.set_xlabel("")
axs.tick_params(axis='y', labelsize=12)
axs.tick_params(axis='x', labelsize=14)
plt.tight_layout()
plt.savefig(result_folder+"Antibody_responses_to homologousOPS_infection_sites.png",transparent=True, dpi=300,bbox_inches='tight')
#============================================================================================#
#============================================================================================#